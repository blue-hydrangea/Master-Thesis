...
# SB: the following path was adapted to the directory provided by Wilhelm:
...
cp /speech/db/eng_US/LVCSR/LDC/TIMIT_LDC93S1/package/timit/TIMIT/DOC/TIMITDIC.TXT data/local/dict
echo "sil  sil" >> data/local/dict/TIMITDIC.TXT
cat data/local/dict/TIMITDIC.TXT | sed "s/\///g" | sed "s/;.*//g" | sed "s/'.*//g"| sed "s/-.*//g" | sed "s/^[a-z]*$//g" | sed "s/\t//g" | sed '/^$/d' > data/local/dict/lexicon.txt
awk -F "  " '{print $2}' data/local/dict/lexicon.txt | sed "s/ /\n/g" | sed '/^$/d' | sort | uniq > data/local/dict/phones.txt
...
for x in train dev test; do
  steps/make_mfcc.sh --nj $feats_nj --cmd "$train_cmd --max-jobs-run 6" data/$x $mfccdir
  steps/compute_cmvn_stats.sh data/$x $mfccdir
done

# SB: took out exp/make_mfcc (before: failed due to missing feats.scp)
...
steps/train_mono.sh  --nj "$train_nj" --cmd "$train_cmd --max-jobs-run 6" data/train data/lang exp/mono
...
local/score_lmwt_wip.sh --cmd "$decode_cmd" --min-lmwt 1 --max-lmwt 50 data/dev exp/mono/graph exp/mono/decode_dev

steps/decode.sh --skip-scoring true --nj "$decode_nj" --cmd "$decode_cmd" \
 exp/mono/graph data/test exp/mono/decode_test

local/score_lmwt_wip.sh --cmd "$decode_cmd" --min-lmwt 1 --max-lmwt 50 data/test exp/mono/graph exp/mono/decode_test

echo ============================================================================
echo "           tri1 : Deltas + Delta-Deltas Training & Decoding               "
echo ============================================================================

steps/align_si.sh --boost-silence 1.25 --nj "$train_nj" --cmd "$train_cmd --max-jobs-run 6" \
 data/train data/lang exp/mono exp/mono_ali

# Train tri1, which is deltas + delta-deltas, on train data.
steps/train_deltas.sh --cmd "$train_cmd --max-jobs-run 6" \
...
local/score_lmwt_wip.sh --cmd "$decode_cmd" --min-lmwt 1 --max-lmwt 50 data/dev exp/tri1/graph exp/tri1/decode_dev

steps/decode.sh --skip-scoring true --nj "$decode_nj" --cmd "$decode_cmd" \
 exp/tri1/graph data/test exp/tri1/decode_test

local/score_lmwt_wip.sh --cmd "$decode_cmd" --min-lmwt 1 --max-lmwt 50 data/test exp/tri1/graph exp/tri1/decode_test
...
steps/align_si.sh --nj "$train_nj" --cmd "$train_cmd --max-jobs-run 6" \
  data/train data/lang exp/tri1 exp/tri1_ali

steps/train_lda_mllt.sh --cmd "$train_cmd --max-jobs-run 6" \
...
local/score.sh --min-lmwt 1 --max-lmwt 20 data/dev exp/tri2/graph exp/tri2/decode_dev

steps/decode.sh --skip-scoring true --nj "$decode_nj" --cmd "$decode_cmd" \
 exp/tri2/graph data/test exp/tri2/decode_test

local/score.sh --min-lmwt 1 --max-lmwt 20 data/test exp/tri2/graph exp/tri2/decode_test
...
steps/train_sat.sh --cmd "$train_cmd --max-jobs-run 6" \
...
local/score.sh --min-lmwt 1 --max-lmwt 20 data/dev exp/tri3/graph exp/tri3/decode_dev

steps/decode_fmllr.sh --skip-scoring true --nj "$decode_nj" --cmd "$decode_cmd" \
 exp/tri3/graph data/test exp/tri3/decode_test

local/score.sh --min-lmwt 1 --max-lmwt 20 data/test exp/tri3/graph exp/tri3/decode_test
...
steps/decode_sgmm2.sh --skip-scoring true --nj "$decode_nj" --cmd "$decode_cmd"\
 --transform-dir exp/tri3/decode_dev exp/sgmm2_4/graph data/dev \
 exp/sgmm2_4/decode_dev

local/score.sh --min-lmwt 1 --max-lmwt 20 data/dev exp/sgmm2_4/graph exp/sgmm2_4/decode_dev

steps/decode_sgmm2.sh --skip-scoring true --nj "$decode_nj" --cmd "$decode_cmd"\
 --transform-dir exp/tri3/decode_test exp/sgmm2_4/graph data/test \
 exp/sgmm2_4/decode_test

local/score.sh --min-lmwt 1 --max-lmwt 20 data/test exp/sgmm2_4/graph exp/sgmm2_4/decode_test
...
steps/nnet2/train_tanh.sh --mix-up 5000 --initial-learning-rate 0.015 --final-learning-rate 0.002 \
--num-hidden-layers 2 --num-jobs-nnet 10 data/train data/lang exp/tri3_ali exp/tri4_nnet

# SB: script was crashing due to problem with queue.pl, training without grid took about an hour
...
echo ============================================================================
echo "                        SB: Summarizing Results                         "
echo ============================================================================

utils/summarize_sclite_output.py exp/mono/decode_dev > SB_Results.txt

utils/summarize_sclite_output.py exp/mono/decode_test >> SB_Results.txt

utils/summarize_sclite_output.py exp/tri1/decode_dev >> SB_Results.txt

utils/summarize_sclite_output.py exp/tri1/decode_test >> SB_Results.txt

utils/summarize_sclite_output.py exp/tri2/decode_dev >> SB_Results.txt

utils/summarize_sclite_output.py exp/tri2/decode_test >> SB_Results.txt

utils/summarize_sclite_output.py exp/tri3/decode_dev >> SB_Results.txt

utils/summarize_sclite_output.py exp/tri3/decode_test >> SB_Results.txt

utils/summarize_sclite_output.py exp/sgmm2_4/decode_dev >> SB_Results.txt

utils/summarize_sclite_output.py exp/sgmm2_4/decode_test >> SB_Results.txt

for iter in 1 2 3 4; do
	utils/summarize_sclite_output.py exp/sgmm2_4_mmi_b0.1/decode_dev_it$iter >> SB_Results.txt
	utils/summarize_sclite_output.py exp/sgmm2_4_mmi_b0.1/decode_test_it$iter >> SB_Results.txt
done

utils/summarize_sclite_output.py exp/tri4_nnet/decode_dev >> SB_Results.txt

utils/summarize_sclite_output.py exp/tri4_nnet/decode_test >> SB_Results.txt

for iter in 1 2 3 4; do
	utils/summarize_sclite_output.py exp/combine_2/decode_dev_it$iter >> SB_Results.txt
	utils/summarize_sclite_output.py exp/combine_2/decode_test_it$iter >> SB_Results.txt
done
